describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let item in prodItems)
    {
      if(item==0)
        continue;
      let shadow = await prodItems[item].getProperty("shadowRoot");
      let button = await shadow.$('button');
      let idk = await button.click();
    }
    let inner = await page.$("#cart-count");
    inner = await inner.getProperty("innerText");
    let value = await inner.jsonValue();
